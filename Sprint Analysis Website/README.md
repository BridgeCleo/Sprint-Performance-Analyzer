...
The website labels each chart with the uploaded CSV filename and includes Download PNG buttons. In comparison mode, it shows individual velocity and acceleration graphs for each sprint plus velocity and acceleration comparison graphs. It also includes a Download analysis JSON button that exports the original rows, calculated values, summary metrics, phase breakdowns, and comparison-ready percent data in one file.
